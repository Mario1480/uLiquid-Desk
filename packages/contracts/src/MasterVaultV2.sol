...
contract MasterVaultV2 {
  ...
  function createBotVault(bytes32 templateId, bytes32 botId, uint256 allocation) external onlyOwner whenNotPaused returns (address botVault) {
    ...
    BotVaultV2(botVault).topUp(allocation);
    freeBalance -= allocation;
    reservedBalance += allocation;
    emit BotVaultProvisioned(botVault, templateId, botId, allocation);
    emit ReservedForBotVault(botVault, allocation, freeBalance, reservedBalance);
  }
  ...
  function treasuryRecipient() public view returns (address) {
    return IMasterVaultFactoryV2(factory).treasuryRecipient();
  }

  function profitShareFeeRatePct() public view returns (uint256) {
    return IMasterVaultFactoryV2(factory).profitShareFeeRatePct();
  }

  function claimFromBotVault(address botVault, uint256 releasedReserved, uint256 grossReturned) external onlyOwner {
    BotVaultV2.Status botStatus = BotVaultV2(botVault).status();
    require(_isClaimableStatus(botStatus), "claim_not_allowed");
    _settleBotVault(botVault, releasedReserved, grossReturned, false);
  }
  ...
  function recoverClosedBotVault(address botVault, uint256 releasedReserved, uint256 grossReturned) external onlyOwner {
    require(isBotVault[botVault], "unknown_bot_vault");
    require(BotVaultV2(botVault).status() == BotVaultV2.Status.CLOSED, "recovery_not_allowed");
    _settleBotVault(botVault, releasedReserved, grossReturned, true);
  }
  ...
  function principalOutstanding(address botVault) public view returns (uint256) {
    uint256 allocated = BotVaultV2(botVault).principalAllocated();
    uint256 returned = BotVaultV2(botVault).principalReturned();
  ...
  function closeBotVault(address botVault, uint256 releasedReserved, uint256 grossReturned) external onlyOwner {
    require(isBotVault[botVault], "unknown_bot_vault");
    require(BotVaultV2(botVault).status() == BotVaultV2.Status.CLOSE_ONLY, "invalid_transition");
    _settleBotVault(botVault, releasedReserved, grossReturned, false);
    BotVaultV2(botVault).close();
  ...
  function setBotVaultCloseOnly(address botVault) external onlyOwner {
    require(isBotVault[botVault], "unknown_bot_vault");
    BotVaultV2(botVault).setCloseOnly();
  }
  ...
  function _allocateToBotVault(address botVault, uint256 amount) private {
    ...
    BotVaultV2(botVault).topUp(amount);
    emit ReservedForBotVault(botVault, amount, freeBalance, reservedBalance);
  }

  function _settleBotVault(address botVault, uint256 releasedReserved, uint256 grossReturned, bool isClosedRecovery) private {
    require(isBotVault[botVault], "unknown_bot_vault");
    ...
    BotVaultV2 vault = BotVaultV2(botVault);
    ...
    if (isClosedRecovery) {
      vault.recordClosedRecoveryRelease(releasedReserved, grossReturned);
    } else {
    ...
    if (isClosedRecovery) {
      emit BotVaultRecovered(botVault, releasedReserved, settlement.netReturned, freeBalance, reservedBalance);
    } else {
      emit BotVaultClaimed(botVault, releasedReserved, settlement.netReturned, freeBalance, reservedBalance);
    }
  }

  function _calculateSettlement(BotVaultV2 vault, uint256 releasedReserved, uint256 grossReturned)
    private
    view
  ...
  function _createBotVault(bytes32 templateId, bytes32 botId, address agentWallet) private returns (address botVault) {
    BotVaultV2 vault = new BotVaultV2(address(this), owner, templateId, botId, agentWallet);
    botVault = address(vault);
    isBotVault[botVault] = true;
    emit BotVaultCreated(botVault, templateId, agentWallet);
  }

  function _isClaimableStatus(BotVaultV2.Status status) private pure returns (bool) {
    return status == BotVaultV2.Status.ACTIVE || status == BotVaultV2.Status.PAUSED || status == BotVaultV2.Status.CLOSE_ONLY;
  }
}
