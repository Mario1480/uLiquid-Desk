...
The payment-custody interface is in scope because the sale depends on it, but the repository does not yet contain the selected production custody implementation. The audit cannot provide end-to-end payment-safety assurance until that implementation and its operational model are added to scope.
...
Round start and end timestamps are owner-configurable only while a round is in `DRAFT` and become frozen at `markReady()`. The production withdrawal period is a constructor parameter and must be frozen before deployment; it is not established by the directory separation.
...
- Select and include the production payment-custody/safeguarding implementation.
