...
type BotDetail = {
  ...
  botVault?: {
    ...
    healthSummary?: {
      ...
      fundingHealth?: string | null;
      onchainStateKnown?: boolean | null;
...
function formatBotVaultFundingStatus(value: string | null | undefined): string {
  ...
  if (!normalized) return "not_created";
  if (normalized === "hyper_evm_funding_requested") return "requested";
  if (normalized === "hyper_evm_confirmed_onchain" || normalized === "hyper_evm_funded") return "confirmed_onchain";
  return normalized;
}

function formatBotVaultHypercoreFundingStatus(value: string | null | undefined): string {
  const normalized = String(value ?? "").trim().toLowerCase();
  if (!normalized) return "-";
  if (normalized === "pending") return "transfer_pending";
  return normalized;
}
...
function BotDetailsPageContent() {
  ...
  async function withdrawAgentHype() {
    ...
    try {
      ...
      setNotice("HYPE withdrawal submitted from the agent wallet.");
      await loadBase();
    } catch (e) {
      setError(errMsg(e));
    } finally {
  ...
  async function fundAgentHype() {
    const targetAddress = String(agentWallet?.address ?? "").trim();
    if (!isConnected || !address) {
      setError("Connect a wallet first to fund the agent wallet.");
      return;
    }
    if (!targetAddress || !isAddress(targetAddress)) {
      setError("Save a valid agent wallet first.");
      return;
    }
    ...
    try {
      ...
      setNotice(`Funding tx submitted: ${String(txHash as Hex).slice(0, 10)}...`);
      await loadBase();
    } catch (e) {
      setError(errMsg(e));
    } finally {
