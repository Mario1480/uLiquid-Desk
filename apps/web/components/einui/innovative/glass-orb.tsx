...
const statusStyles: Record<GlassOrbStatus, StatusConfig> = {
  idle: {
    ring: "ein:border-cyan-300/20",
    glow: [
      "radial-gradient(circle at ein:top left, rgba(255,255,255,0.3), transparent 35%)",
      "radial-gradient(circle at ein:bottom right, rgba(34,211,238,0.24), transparent 28%)",
  ...
  active: {
    ring: "ein:border-blue-200/30",
    glow: [
      "radial-gradient(circle at ein:top left, rgba(255,255,255,0.34), transparent 35%)",
      "radial-gradient(circle at ein:bottom right, rgba(96,165,250,0.3), transparent 28%)",
  ...
  loading: {
    ring: "ein:border-white/25",
    glow: [
      "radial-gradient(circle at ein:top left, rgba(255,255,255,0.32), transparent 35%)",
      "radial-gradient(circle at ein:bottom right, rgba(103,232,249,0.3), transparent 28%)",
...
const GlassOrb = React.forwardRef<HTMLDivElement, GlassOrbProps>(
  (
      ...
      const animate = () => {
        ...
        if (showTrails) {
          ...
          if (
            ...
            el.className = "absolute ein:rounded-full ein:bg-white/70 ein:blur-[2px] ein:pointer-events-none"
    ...
    return (
      <div
        ref={rootRef}
        className={cn("ein:relative ein:overflow-hidden ein:pointer-events-none", className)}
        {...props}
      ...
      >
        <div
          ref={orbRef}
          className={cn(
            "ein:absolute ein:rounded-full",
            "ein:border ein:border-white/10 ein:shadow-[0_0_120px_rgba(56,189,248,0.15)]",
            config.ring,
            config.pulse && "ein:animate-pulse",
          )}
          style={{
            width: sizePx,
            height: sizePx,
        ...
        >
          <div
            className="ein:absolute ein:inset-0 ein:rounded-full ein:border ein:border-white/10"
            style={{ background: config.glow }}
          />

          <div
            className="ein:absolute ein:inset-4 ein:rounded-full ein:bg-white/10 ein:blur-[2px]"
            style={{
              background: "radial-gradient(circle at 35% 35%, rgba(255,255,255,0.55), transparent 45%)",
            }}
          />

          <div className="ein:absolute ein:inset-0 ein:rounded-full ein:mix-blend-screen ein:opacity-80" />
          ...
          <div
            className="ein:absolute ein:inset-0 ein:rounded-full"
            style={{
              background:
          ...
          {label ? (
            <div className="ein:absolute ein:bottom-3 ein:left-1/2 ein:-translate-x-1/2 ein:rounded-full ein:bg-black/40 ein:px-3 ein:py-1 ein:text-[11px] ein:uppercase ein:tracking-[0.2em] ein:text-white/85 ein:shadow-[0_0_25px_rgba(0,0,0,0.2)]">
              {label}
            </div>
          ) : null}
        </div>
        ...
        <div ref={particleLayerRef} className="ein:absolute ein:inset-0 ein:overflow-hidden ein:pointer-events-none" />
      </div>
    )
  },
)
