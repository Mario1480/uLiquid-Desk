...
export default function Gallery() {
 ...
 return <section className="ein-gallery uiPage">
  ...
  <div className="ein-demo-row"><label>{t("component")}<select data-ein-control="true" className="input" value={selected} onChange={e=>{setSelected(e.target.value);setMessage("");requestAnimationFrame(()=>preview.current?.scrollIntoView({block:"start",behavior:"instant"}));}}>{entries.map(item=><option value={item.name} key={item.name}>{item.title} — {item.name}</option>)}</select></label><label>{t("theme")}<select data-ein-control="true" className="input" value={theme} onChange={e=>setTheme(e.target.value as EinTheme)}><option value="ocean">Ocean</option><option value="aurora">Aurora</option><option value="forest">Forest</option></select></label><GlassButton onClick={()=>setPaused(v=>!v)}>{paused?t("resume"):t("pause")}</GlassButton></div>
  <p>{t("samples")}</p>
