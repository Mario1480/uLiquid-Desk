...
export const DeskBadge = forwardRef<HTMLSpanElement, ComponentPropsWithoutRef<"span">>(function DeskBadge({ className, ...props }, ref) {
  ...
  const variant = /danger|error|destructive|badgeShort|ChipShort|StatusCancelled|Pillfailed|Pillexpired|calendarImpactBadgeHigh/i.test(classes) ? "destructive"
    : /warn|Pillreview_required|calendarImpactBadgeMedium/i.test(classes) ? "warning"
    : /success|badgeOk|badgeLong|ChipLong|StatusFilled|Pillpaid|newsBadgeCrypto|calendarImpactBadgeLow/i.test(classes) ? "success"
