...
export default function AgentContextBar({ context, profiles, accounts, activeProfile, skillCount, disabled, onChange, onOpenSkills }: {
  ...
  return (
    <div className="agentChatContextBar" aria-label={t("context.ariaLabel")}>
      ...
      {accountRead ? <label><span>{t("context.account")}{accountRequired ? <em className="agentChatRequiredFlag">{t("context.required")}</em> : null}</span><select ref={accountSelectRef} className={`input${accountRequired ? " agentChatAccountSelectRequired" : ""}`} required aria-invalid={accountRequired} disabled={disabled} value={context.selectedExchangeAccountId ?? ""} onChange={(event) => onChange({ selectedExchangeAccountId: event.target.value || null })}><option value="">{t("context.selectAccount")}</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.exchange.toUpperCase()} · {account.label}</option>)}</select></label> : null}
      ...
      <label><span>{accountRead ? t("context.symbolFilter") : t("context.symbol")}</span><input className="input" disabled={disabled} value={context.symbol ?? ""} placeholder={accountRead ? t("context.allPositions") : undefined} maxLength={32} onChange={(event) => onChange({ symbol: event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "") || null })} /></label>
