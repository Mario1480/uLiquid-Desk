  ...
  return (
    <div className="agentChatBlocks">
      {blocks.map((block, index) => {
        ...
        if (block.type === "summary") return <section className="agentChatBlock" key={key}><strong>{block.title}</strong><p>{block.text}</p></section>;
        ...
        if (block.type === "risk_findings") return (
          <section className="agentChatBlock" key={key}>
            <div className="agentChatBlockTitle"><AppIcon name="risk" /> {block.title}<span className={`badge agentChatRisk-${block.riskLevel}`}>{block.riskLevel}</span></div>
        ...
        if (block.type === "scenario_table") return (
          <section className="agentChatBlock agentChatTableBlock" key={key}>{block.title ? <strong>{block.title}</strong> : null}<div className="agentChatTableWrap"><table><thead><tr>{block.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{block.rows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={`${rowIndex}:${cellIndex}`}>{cell}</td>)}</tr>)}</tbody></table></div></section>
        );
        if (block.type === "prediction_comparison") return <section className="agentChatBlock" key={key}>{block.title ? <strong>{block.title}</strong> : null}<dl className="agentChatComparison"><div><dt>{t("blocks.prediction")}</dt><dd>{block.prediction}</dd></div><div><dt>{t("blocks.position")}</dt><dd>{block.position}</dd></div><div><dt>{t("blocks.divergence")}</dt><dd>{block.divergence}</dd></div></dl></section>;
        return <section className="agentChatBlock" key={key}>{block.title ? <strong>{block.title}</strong> : null}<Sources sources={block.sources} degradedLabel={t("states.degraded")} staleLabel={t("states.stale")} /></section>;
      })}
    </div>
  );
}
