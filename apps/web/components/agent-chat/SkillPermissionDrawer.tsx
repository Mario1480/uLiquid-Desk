  ...
  return (
    <div className="agentChatDrawerBackdrop" role="presentation" onMouseDown={onClose}>
      <section className="agentChatDrawer" role="dialog" aria-modal="true" aria-labelledby="agent-skills-title" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="agentChatDrawerSection"><h3>{t("skills.enabled")}</h3><div className="agentChatSkillList">{skills.filter((skill) => enabled.has(skill.id)).map((skill) => <article key={skill.id}><span className="agentChatSkillIcon"><AppIcon name={skill.category === "risk" ? "risk" : skill.category === "portfolio" ? "accounts" : "ai"} /></span><div><strong>{skill.id}</strong><p>{t(`skills.categoryDescriptions.${skill.category}`)}</p></div><span className="badge">{skill.accessLevel === "account_read" ? t("access.readOnly") : t("access.publicData")}</span></article>)}</div></div>
