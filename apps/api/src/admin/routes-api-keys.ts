...
export function registerAdminApiKeyRoutes(app: express.Express, deps: RegisterAdminApiKeyRoutesDeps) {
  ...
  app.get("/admin/settings/api-keys/status", requireAuth, async (_req, res) => {
    ...
    if (effectiveProvider.provider === "disabled") {
      return res.json({ ok: false, status: "error", source: resolved.source, checkedAt, message: "AI provider is disabled.", model: effectiveModel.model, provider: effectiveProvider.provider, baseUrl: effectiveBaseUrl.baseUrl });
    }
    if (resolved.decryptError) {
      return res.json({ ok: false, status: "error", source: resolved.source, checkedAt, message: "Stored AI key could not be decrypted.", model: effectiveModel.model, provider: effectiveProvider.provider, baseUrl: effectiveBaseUrl.baseUrl });
    }
    if (!resolved.apiKey) {
      return res.json({ ok: false, status: "missing_key", source: resolved.source, checkedAt, message: "No AI API key configured.", model: effectiveModel.model, provider: effectiveProvider.provider, baseUrl: effectiveBaseUrl.baseUrl });
    }
    ...
    try {
      ...
      const isOpenAiGpt5Model = effectiveProvider.provider === "openai" && effectiveModel.model.startsWith("gpt-5");
      const healthPayload: Record<string, unknown> = {
        model: effectiveModel.model,
        messages: [{ role: "user", content: "ping" }],
        ...(isOpenAiGpt5Model ? { max_completion_tokens: 8 } : { temperature: 0, max_tokens: 8 })
      };
      const headers = { "Content-Type": "application/json", Authorization: `Bearer ${resolved.apiKey}` };
      const body = JSON.stringify(healthPayload);
      const doFetch = (url: string) => fetch(url, { method: "POST", headers, body, signal: controller.signal });
      let response: Response;
      try {
        response = await doFetch(endpoint);
      } catch (error) {
        const tryDockerFallback = effectiveProvider.provider === "ollama" && /^https?:\/\/(localhost|127\.0\.0\.1|\[::1\])(?::\d+)?(\/|$)/i.test(effectiveBaseUrl.baseUrl) && !controller.signal.aborted;
      ...
      return res.json({ ok: false, status: "error", source: resolved.source, checkedAt, latencyMs: Date.now() - startedAt, message: isAbort ? "Connection timed out." : String(error), model: effectiveModel.model, provider: effectiveProvider.provider, baseUrl: effectiveBaseUrl.baseUrl });
    } finally {
  ...
  app.get("/admin/settings/api-keys/ccpay-status", requireAuth, async (_req, res) => {
    ...
    if (resolved.decryptError) {
      return res.json({ ok: false, status: "error", source: resolved.source, checkedAt, message: "Stored CCPay credentials could not be decrypted.", hasAppId: Boolean(resolved.appId), hasAppSecret: Boolean(resolved.appSecret), baseUrl: resolved.baseUrl, priceFiatId: resolved.priceFiatId, webBaseUrl: resolved.webBaseUrl, sources: { appId: resolved.appIdSource, appSecret: resolved.appSecretSource, baseUrl: resolved.baseUrlSource, priceFiatId: resolved.priceFiatIdSource, webBaseUrl: resolved.webBaseUrlSource }, missingFields });
    }
    if (!resolved.isConfigured) {
      return res.json({ ok: false, status: "missing_config", source: resolved.source, checkedAt, message: "CCPayments configuration is incomplete.", hasAppId: Boolean(resolved.appId), hasAppSecret: Boolean(resolved.appSecret), baseUrl: resolved.baseUrl, priceFiatId: resolved.priceFiatId, webBaseUrl: resolved.webBaseUrl, sources: { appId: resolved.appIdSource, appSecret: resolved.appSecretSource, baseUrl: resolved.baseUrlSource, priceFiatId: resolved.priceFiatIdSource, webBaseUrl: resolved.webBaseUrlSource }, missingFields });
    }
    return res.json({ ok: true, status: "ok", source: resolved.source, checkedAt, message: "CCPayments configuration is ready.", hasAppId: true, hasAppSecret: true, baseUrl: resolved.baseUrl, priceFiatId: resolved.priceFiatId, webBaseUrl: resolved.webBaseUrl, sources: { appId: resolved.appIdSource, appSecret: resolved.appSecretSource, baseUrl: resolved.baseUrlSource, priceFiatId: resolved.priceFiatIdSource, webBaseUrl: resolved.webBaseUrlSource }, missingFields });
  });
  ...
  app.get("/admin/settings/api-keys/salad-runtime/status", requireAuth, async (_req, res) => {
    ...
    if (result.ok) logger.info("salad_runtime_status_ok", { target: resolvedConfig.config, latency_ms: result.latencyMs, state: result.state });
    else logger.warn("salad_runtime_status_failed", { target: resolvedConfig.config, latency_ms: result.latencyMs, state: result.state, error_code: result.errorCode, http_status: result.httpStatus, message: result.message });
    return res.status(statusCode).json({ ...result, source: resolvedKey.source, target: resolvedConfig.config });
  });
  ...
  app.get("/admin/settings/api-keys/fmp-status", requireAuth, async (_req, res) => {
    ...
    const checkedAt = new Date().toISOString();
    if (resolved.decryptError) {
      return res.json({ ok: false, status: "error", source: resolved.source, checkedAt, message: "Stored FMP key could not be decrypted." });
    }
    if (!resolved.apiKey) {
      return res.json({ ok: false, status: "missing_key", source: resolved.source, checkedAt, message: "No FMP API key configured." });
    }
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 8_000);
    const startedAt = Date.now();
    try {
      await deps.fetchFmpEconomicEvents({ apiKey: resolved.apiKey, baseUrl: process.env.FMP_BASE_URL, from: "2026-01-01", to: "2026-01-02", signal: controller.signal });
      return res.json({ ok: true, status: "ok", source: resolved.source, checkedAt, latencyMs: Date.now() - startedAt, message: "FMP connection is healthy." });
    } catch (error) {
      const isAbort = error instanceof Error && error.name === "AbortError";
      ...
      return res.json({ ok: false, status: "error", source: resolved.source, checkedAt, latencyMs: Date.now() - startedAt, ...(httpStatus ? { httpStatus } : {}), message });
    } finally {
      clearTimeout(timeout);
    }
  });
