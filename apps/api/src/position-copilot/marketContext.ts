...
import { sharedMarketStore, normalizeSharedCandles, providerObservedAt, type SharedMarketRead } from "../market-data/sharedMarket.js";
...
export async function loadPositionMarketContext(params: {
  ...
  if (params.marketType !== "perp") return buildMarketFeatureContext([], [], ["spot_market_features_not_integrated"]);
  ...
  const market = { providerId: capability.providerId, sourceVenue, marketType: "perp" as const, symbol: params.symbol };
  ...
  await Promise.all([
    (async () => {
      ...
      const read = await sharedDerivativesStore.read(market, () => withClient(client => client.getDerivativesSnapshot(params.symbol)));
    ...
    (async () => {
      if (capability.marketData.candles === "unsupported") { warnings.push("candles_unsupported"); return; }
      const read = await sharedMarketStore.read<"candles">({ ...market, schemaVersion: "1.0.0", dataset: "candles", interval: "1h", limit: 100 },
        () => withClient(async client => normalizeSharedCandles(await client.getCandles({ symbol: params.symbol, timeframe: "1h", limit: 100 }), "1h", 100)));
      addSnapshot(read);
    ...
    (async () => {
      if (capability.marketData.orderbookAnalytics === "unsupported" || capability.marketData.orderbook === "unsupported") { warnings.push("orderbook_unsupported"); return; }
      const read = await sharedMarketStore.read<"orderbook">({ ...market, schemaVersion: "1.0.0", dataset: "orderbook", limit: 25 },
        () => withClient(async client => {
          ...
          return { data: { bids: depth.bids.slice(0, 25) as Array<[number, number]>, asks: depth.asks.slice(0, 25) as Array<[number, number]> }, observedAt: providerObservedAt(depth.ts), warnings: [] };
        }));
      addSnapshot(read);
