...
export const FEATURE_CONTEXT_POLICY = [
  ...
  "Funding/OI are current snapshots only: no changes, trends, percentiles, Z-scores, crowding or extreme-funding claims without comparable history. A simple annualized funding estimate is not a forecast or yield promise.",
...
export function buildMarketFeatureContext(snapshots: MarketSnapshotEvidence[], features: StoredFeatureEvidence[], warnings: string[] = []): MarketFeatureContext {
  const manifest = z.array(marketSnapshotEvidenceSchema).max(3).parse(snapshots);
  const parsed = parseStoredFeatures(features);
  if (features.length > 4 || parsed.length !== features.length || parsed.some(f => !manifest.some(s => featureMatchesSnapshot(f, s)))) {
  ...
  const observed = manifest.filter(s => s.dataset !== "candles" && s.observedAt).map(s => Date.parse(s.observedAt!));
