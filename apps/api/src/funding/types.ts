...
export type TransferCapabilityDirection = "core_to_evm" | "evm_to_core";

export type TransferCapabilityAsset = "USDC" | "HYPE";
...
export type TransferCapability = {
  id: string;
  direction: TransferCapabilityDirection;
  asset: TransferCapabilityAsset;
  supported: boolean;
...
export type WalletFundingOverview = {
  ...
  externalLinks: FundingExternalLinksResponse["links"];
  updatedAt: string;
};
...
export type FundingHistorySourceItem = {
  id: string;
  actionType: string;
  status: string;
  txHash: string | null;
...
export type FundingReadService = {
  getFundingOverview(params: { address: string }): Promise<WalletFundingOverview>;
