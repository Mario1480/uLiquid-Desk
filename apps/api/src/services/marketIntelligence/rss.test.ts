...
import { assertPublicDns, fetchBoundedFeed, isPublicIpAddress, resolveFeedUserAgent, validateFeedUrl } from "./providers/rss/security.js";
...
test("bounded feed timeout also aborts stalled public DNS validation", async () => {
  ...
  await assert.rejects(
    fetchBoundedFeed({
      ...
      timeoutMs: 20,
      dnsLookupAll: async () => new Promise(() => undefined)
    }),
    /rss_request_timeout/
  );
