...
Current classification: `IMPLEMENTED / PRODUCTION USE EVIDENCED / FORMAL CLOSEOUT OPEN`.

The schema and API now contain the credit ledger, pricing revisions, agent runs,
usage records, reservations, feature flags, routing, user/admin surfaces, and
reconciliation support described by this packet. Later production Agent Chat
records include settled AI Credit usage. The implementation workstreams below
are therefore retained as design history, not as an unstarted task list.

This pass did not query the live database. Current pricing approval, a live
reservation and usage census, failure/concurrency evidence, monitoring ownership,
Deep Analysis scope, and final billing reconciliation remain open. See the
...
Use `CODEX-MASTER-PROMPT.md` only when changing the implementation. This packet
remains active for current pricing verification, billing reconciliation,
operations acceptance, and final closeout evidence.
